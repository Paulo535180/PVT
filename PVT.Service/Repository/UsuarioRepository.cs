...
namespace PVT.Service.Repository
{
    public class UsuarioRepository : IUsuarioRepository
    {
		...
		PVT_USUARIO_GESTOR.ID as ID_GESTOR
        ...
        public async Task<IList<Usuario>> Listagem()
        {
            var lista = await _connection.QueryAsync<Usuario>($@"
        select UUSUARIO.ID as ID,
        Nome AS NOME,
        email AS EMAIL,
        UUSUARIO.SENHA AS Password,
        ...
        where USISTEMA.SIGLA = 'PVT' 
			and UUSUARIO.ATIVO = 'T' 
			and UPERFIL.PERFIL != 'ADMINISTRADOR' 
			and PVT_USUARIO_GESTOR.ID_SETOR IS NULL");
            return lista.ToList();
        }
    }
}
